...
namespace Gifter.Repositories
{
    public class PostRepository : BaseRepository, IPostRepository
    {
        ...
        public List<Post> SearchByTitle(string criterion, bool sortDescending)
        {
            using (var conn = Connection)
            {
                conn.Open();
        ...
        public List<Post> SearchByTitleAndCaption(string criterion, string taco, bool sortDescending)
        {
            using (var conn = Connection)
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    FROM Post p 
                        LEFT JOIN UserProfile up ON p.UserProfileId = up.id
                    WHERE p.Title LIKE @Criterion AND p.Caption LIKE @taco";
                    ...
                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@Criterion", $"%{criterion}%");
                    DbUtils.AddParameter(cmd, "@taco", $"%{taco}%");
                    var reader = cmd.ExecuteReader();
